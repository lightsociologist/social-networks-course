...
Note: You can also accomplish a lot of visualization tasks in [Gephi](https://gephi.org/), a point-and-click program. There are downstream limitations for Gephi in terms of statistical analysis and it presents some replication concerns, but it can generate a useful visualization.

## Visualization using `igraph`

First, we need to load the `igraph` (the workhorse for the course), `networkD3` (for interactive graphs), and `ggraph` (for network viz) package into R. `ggforce` is a package to add elements to `ggraph`.
...
toy.g <- graph_from_adjacency_matrix(toy.undirected, mode="max")

```

Now we use `igraph` notation - V(g)\$thing \<- thing - to store node attributes in the igraph object if both objects are ordered the same way.
...
You can also write edge lists directly into `igraph` using `make_graph()`. `igraph` will read each pair as an edge. Here we have indicated an undirected graph by directed=F.

```{r, echo=TRUE}
edge.list <- c(1,3, 2,3, 3,4, 4,5, 4,6, 5,6)

exa_g <- make_graph(edge.list, directed=F)	
V(exa_g)$attr1 <- c(1,1,1,0,0,0)		# attach the attribute vector
plot.igraph(exa_g, vertex.color=V(exa_g)$attr1*2)	# how’s it look?
```

We can also add multiple kinds of edges (and other attributers) in the same way as node attributes, but calling edges instead of vertices: E(g)\$attribute \<- Attribute. Or like this:
...
### Bringing data in `igraph` using `read_graph`

We can bring data into `igraph` via other programs like the networks program [Pajek](http://mrvar.fdv.uni-lj.si/pajek/). Pajek is a popular program used to visualize and analyze social networks. The file extensions for Pajek include .net for the network file and .clu for attributes which are often kept in separate files.

`read_graph()` will read networks from other programs. format= identifies the type of file (e.g. the program in which the file was generated, including Pajek and Gephi).
...
We can use `print` for an overview of the `igraph` object. Here is the documentation on how to read the overview.
...
The Valente classroom graph consists of 37 nodes or students with 145 ties between them. It is a directed graph (D) with names (N). It is not weighted or bipartite. There is a vector attributes for ids and names.
...
The most annoying default for R is the arrow size in directed graphs. You can change that with edge.arrow.size=.25

```{r fig.align='center', echo=TRUE}

plot.igraph(valente, vertex.color=V(valente)$colors, 
            vertex.size=V(valente)$node.size*2, edge.arrow.size=.25)
...
The `layout=()` function allows you to specify which layout you would like to use. Perhaps you would like to begin by visualizing your network as a circle. `layout=layout_in_circle` will work.
...
#### Fruchterman-Reingold - Force-directed alogrithm

The Fruchterman-Reingold layout, `layout=layout_with_fr`, operates with a physics metaphor. Nodes repel one another and edges act like springs that push the nodes together. This push and pull occurs iteratively until the "system" is balanced. The end result is that the nodes that are connected or share edges cluster together. This layout is useful for small to medium graphs.
...
The grid layout, `layout=layout.grid`, is a basic layout that just flattens all of the connections into an grid.
...
The Kamada-Kawai layout, `layout=layout_with_kk`, emphasizes shortest path distance between nodes. You get a more graph theoretic layout (e.g., distance is more mathematically meaningful), but trade visual clarity with larger graphs.
...
MDS layouts, `layout=layout.mds`, also emphasize shortest paths.
...
#### A note on additional layouts

These layout algorithms can struggle with medium to large size networks. `igraph` includes several algorithms better suited for these tasks that are worth exploring if your network is of a decent size (see `layout_with_drl()` and `layout_with_lgl()`. The default layout in the event that you don't specify is `layout_nicely()` that is a wrapper for the collection of layouts and uses information from the graph to decide the "best" option.
...
I've collected data on Star Wars movies from imdb.com. I copied and pasted the top 15 actors in each of the first six movies into excel and saved as a csv. We can read the csv into a data frame and then use `graph_from_data_frame()` to read into `igraph`.
...
You can see that `igraph` is not reading the graph as bipartite as a B would be in the third slot. We need to use `bipartite_mapping()` to identify the two modes.

```{r, echo=TRUE}
bipartite_mapping(sw.g)

V(sw.g)$type <- bipartite_mapping(sw.g)$type
```

We can see that the different modes are identified by True or False. And we can set up different attributes like color or shape to distinguish between the two modes of actors and movies.
...
We can also use the `networkD3` package to easily visualize interactive networks. You can read an `igraph object` into `networkD3` with the `igraph_to_networkD3` function.
...
Obviously we would want to play around with the settings to properly display labels etc.
