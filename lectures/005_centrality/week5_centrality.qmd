...
Graph Level measures: Centralization - Most centralization scores are normalized so that 1 is maximum centralization or where all the ties head to a single node (e.g,. a star). Feel free to play around with this by using the `make star` function in `igraph`. 
...
![Borgatti Centrality Framework](images/borgatti_centrality_frame.JPG)
...
We can also construct degree as a graph property capturing the extent to which the graph is controlled by a single node: `centr_degree` in `igraph`. Remember that this approaches 1 the extent to which it is a star.

```{r  message=FALSE, echo=TRUE}

centr_degree(flomarriage, normalized=T)
...
Let's check against a star and a completed graph.

First, the star:

```{r  message=FALSE, echo=TRUE}

star.g <- make_star(vcount(flomarriage), mode="undirected")

centr_degree(star.g, normalized=T)
...
full.g <- make_full_graph(vcount(flomarriage), directed=F)

centr_degree(full.g, normalized=T)
...
This can be an interesting way to think about kind of subgraph groupiness as a type of centrality. The k-core is the maximal subgraph where each vertex has at least k degrees. So, each vertex is assigned to its maximum core (where k+1) or where it belongs to k, but not k+1. This is useful for very large graphs like Gonzales-Bailon et al. (2011).
