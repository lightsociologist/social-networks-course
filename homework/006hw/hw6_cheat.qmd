...
bicomponent.org <- biconnected_components(orgnet)

b1 <- bicomponent.org$components[[max(length(bicomponent.org$components))]]

bi.g <- induced_subgraph(graph=orgnet,vids=b1)
...
plot(clique2, vertex.color=V(clique2)$region, edge.arrow.size=.1, main="Consultancy Clique (Region)")

plot(clique2, vertex.color=V(clique2)$gender, edge.arrow.size=.1, main="Consultancy Clique (Gender)")
...
art_points <- articulation_points(orgnet)

rem <- delete_vertices(orgnet, art_points)
...
plot(clique2, vertex.color=V(orgnet)$gender+1, edge.arrow.size=.1, main="Consultancy Clique (Gender)")
...
leiden_large=modularity(bi.g, cluster_leiden(bi.g, resolution = 2)$membership), 
