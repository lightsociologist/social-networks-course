...
coauth.2mode <- graph_from_biadjacency_matrix(coauth)

#Make a one-mode projection of this matrix looking at columns (Author X Author)

coauth.1mode <- bipartite_projection(coauth.2mode, multiplicity=TRUE, which=TRUE)
...
coauth.noiso <- delete_vertices(coauth.1mode, which(degree(coauth.1mode)==0))
...
# Not good practice to introduce here, but this part is extra in multiple ways.
